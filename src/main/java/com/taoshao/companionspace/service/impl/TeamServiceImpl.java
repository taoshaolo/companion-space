...
import com.taoshao.companionspace.contant.TeamConstant;
import com.taoshao.companionspace.exception.BusinessException;
import com.taoshao.companionspace.mapper.TeamMapper;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team> implements TeamService {
    private static final String SALT = "taoshao_team";
    private static final String BY_TEAM_IDS = String.format("companionspace:team:getTeamListByTeamIds:%s", "byTeamIds");
    private static final String TEAMS_KEY = String.format("companionspace:team:getTeams:%s", "getTeams");
    @Resource
    ...
    public TeamUserVo teamQuery(TeamQueryRequest teamQueryRequest, HttpServletRequest request) {
        ...
        String teamQueryKey = String.format("companionspace:team:teamQuery:%s", searchText);
        ValueOperations<String, Object> valueOperations = redisTemplate.opsForValue();
    ...
    public User joinTeam(TeamJoinRequest joinTeam, User loginUser) {
        ...
        RLock lock = redissonClient.getLock("companionspace:join_team");
        try {
            // 抢到锁并执行
            while (true) {
                if (lock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
                    ...
                    Set<Long> userIdList = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(usersId);
                    // 当前队伍是不是已经满人了
                    ...
                    Set<Long> loginUserTeamIdList = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(teamIds);
                    ...
                    String teamIdKey = String.format("companionspace:team:getUsersByTeamId:%s", team.getId());
    ...
    public boolean createTeam(TeamCreateRequest teamCreateRequest, User loginUser) {
        ...
        String teamIds = user.getTeamIds();
        RLock lock = redissonClient.getLock("companionspace:create_team");
        try {
            // 抢到锁并执行
            while (true) {
                if (lock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
                    Gson gson = new Gson();
                    Set<Long> teamIdList = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(teamIds);
                    ...
                    Set<Long> usersIdList = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(usersId);
    ...
    public Boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        if (updateStatus) {
            String teamIdKey = String.format("companionspace:team:getUsersByTeamId:%s", oldTeam.getId());
            deleteRedisKey(teamIdKey);
        }
        return updateStatus;
    }
    ...
    public Boolean kickOutTeamByUserId(KickOutUserRequest kickOutUserRequest, User loginUser) {
        ...
        if (!userService.isAdmin(loginUser) || team.getUserId().equals(userId) || !loginUser.getId().equals(team.getUserId())) {
            throw new BusinessException(ErrorCode.KICK_OUT_USER, "权限不足");
        }
        User user = userService.getById(userId);
        // 当前队伍中的用户id列表
        Set<Long> userIds = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(team.getUsersId());
        // 踢出的用户当前的队伍id列表
        Set<Long> teamIds = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(user.getTeamIds());
        // 用户和队伍都删除各自的id
    ...
    public Boolean transferTeam(TransferTeamRequest transferTeamRequest, User loginUser) {
        ...
        if (!userService.isAdmin(loginUser) && !loginUser.getId().equals(team.getUserId())) {
            throw new BusinessException(ErrorCode.KICK_OUT_USER, "权限不足");
        }
        // 队伍的创建者修改为新用户
    ...
    public boolean dissolutionTeam(Long teamId, HttpServletRequest request) {
        ...
        if (dissolutionTeam) {
            String teamIdKey = String.format("companionspace:team:getUsersByTeamId:%s", teamId);
            deleteRedisKey(teamIdKey);
            redisTemplate.delete(userService.redisFormat(loginUser.getId()));
        }
    ...
    public TeamVo getUsersByTeamId(Long teamId, HttpServletRequest request) {
        ...
        // 当前用户加入的队伍的id
        Set<Long> userTeamIdSet = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(userTeamIds);
        // 当前队伍被加入的用户的id
        Set<Long> usersIdSet = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(usersId);

        // 当前用户不是管理员
        // 当前用户加入的队伍的ids中不包含传过来的队伍id
        // 当前用户的id不等于队伍的创建者id 说明没权限
        boolean noPermissions = !userService.isAdmin(loginUser) && !userTeamIdSet.contains(teamId) && !loginUser.getId().equals(userId);
        if (noPermissions) {
    ...
    public TeamUserVo teamSet(List<Team> teamList) {
        ...
        listTeam.forEach(team -> {
            ...
            Set<Long> userSet = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(usersId);
