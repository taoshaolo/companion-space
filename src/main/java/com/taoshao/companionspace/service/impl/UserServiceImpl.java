...
import com.taoshao.companionspace.contant.UserConstant;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public String redisFormat(Long key) {
        return String.format("companionspace:user:search:%s", key);
    }
    ...
    public List<User> getFriendsById(User currentUser) {
        ...
        Set<Long> friendsId = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(loginUser.getUserIds());
    ...
    public boolean deleteFriend(User currentUser, Long id) {
        ...
        Set<Long> friendsId = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(loginUser.getUserIds());
        //好友的好友列表
        Set<Long> fid = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(friendUser.getUserIds());
        //相当于 互删好友
        friendsId.remove(id);
    ...
    public List<User> searchFriend(UserQueryRequest userQueryRequest, User currentUser) {
        ...
        Set<Long> friendsId = com.taoshao.companionspace.utils.StringUtils.stringJsonListToLongSet(user.getUserIds());
    ...
    public boolean isAdmin(HttpServletRequest request) {
        // 仅管理员可查询
        Object objUser = request.getSession().getAttribute(UserConstant.LOGIN_USER_STATUS);
    ...
    public int updateUser(User user, User currentUser) {
        ...
        if (!StringUtils.isBlank(user.getUserDesc()) && user.getUserDesc().length() > 30) {
        ...
        if (!StringUtils.isBlank(user.getContactInfo()) && user.getContactInfo().length() > 18) {
