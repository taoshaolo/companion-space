package com.taoshao.companionspace.job;
...
import com.taoshao.companionspace.utils.StringUtils;
...
import static com.taoshao.companionspace.contant.FriendConstant.AGREE_STATUS;
import static com.taoshao.companionspace.contant.FriendConstant.EXPIRED_STATUS;
...
public class CacheWarming {
    ...
    public void searchUserList() {
        RLock rLock = redissonClient.getLock("companionspace:cache:searchUserList:lock");
        try {
    ...
    public void searchTeamList() {
        RLock rLock = redissonClient.getLock("companionspace:cache:searchTeamList:lock");
        try {
            if (rLock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
                List<Team> list = teamService.list();
                TeamUserVo teamUserVo = teamService.teamSet(list);
                try {
                    redisTemplate.opsForValue().set("companionspace:team:getTeams:getTeams", teamUserVo, 1 + RandomUtil.randomInt(1, 5), TimeUnit.MINUTES);
                } catch (Exception e) {
                    log.error("redis set key error", e);
                }
            }
    ...
    public void dissolutionExpiredTeam() {
        RLock rLock = redissonClient.getLock("companionspace:cache:dissolutionExpiredTeam:lock");
        try {
            if (rLock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
    ...
    public void chatRecords() {
        RLock rLock = redissonClient.getLock("companionspace:cache:chatRecords:lock");
        try {
            if (rLock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
    ...
    public void isExpires() {
        RLock rLock = redissonClient.getLock("companionspace:cache:isExpires:lock");
        try {
            if (rLock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
